...
export default function ShoppingList({
  ...
  const confirmDelete = async () => {
    ...
    setItems((prev) =>
      prev.filter((i) => i.id !== targetDelete)
    )
  ...
  const IconComponent = (Icons as any)[list.icon] || ArrowLeft
...
>
  <IconComponent
    size={48}
    style={{ color: list.icon_color || '#6B7280' }}
    className="align-top"
  />
  <span className="align-top">{list.name}</span>
</motion.h2>
      ...
      <ul className="space-y-4">
        <AnimatePresence>
          {items.map((item) => (
            ...
            >
              <span className="text-secondary text-lg truncate">
                {item.name}
              </span>
              <motion.button
